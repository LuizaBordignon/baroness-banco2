  ...
  try {
    const result = await pool.query(
      `SELECT
          ...
          b.price + COALESCE(SUM(bc.quantity * p.price), 0) AS price,
          b.status
      ...
      LEFT JOIN booking_charges bc ON bc.id_booking = b.id
      LEFT JOIN products p ON p.id = bc.id_product
      GROUP BY b.id, g.name, r.room_number, e.name
